...
type DigestState = { digest: DetailedDigest | ThematicDigest | null; type: 'detailed' | 'thematic' | null; error: string | null; isLoading: boolean; loadingMessage: string | null; };
...
const getPrunedReadArticleIds = (): Map<string, number> => {
    const storedIds = getStoredData<Map<string, number>>(READ_ARTICLES_KEY, new Map(), (k, v) => k === "" ? new Map(Array.isArray(v) ? v : undefined) : v);
...
const getInitialView = (): CurrentView => {
    ...
    if (type === 'keyword-articles') return { type: 'keyword-articles', value: decodeURIComponent(value) };
    ...
    if (type === 'tag') {
        if (valueParts.length === 2 && (valueParts[0] === 'youtube' || valueParts[0] === 'rss')) {
            return { type: 'tag', value: { name: decodeURIComponent(valueParts[1]), type: valueParts[0] } };
        }
...
const generateArticleMarkdown = async (article: Article): Promise<string> => {
    let articleMarkdown = article.link ? `## [${article.title}](${article.link})\n\n` : `## ${article.title}\n\n`;
    if (article.imageUrl) {
    ...
    if (article.structuredSummary) {
        ...
        const textContent = rawContent.replace(/<[^>]*>?/gm, ' ').replace(/\s+/g, ' ').trim();
        if (textContent) {
...
interface AppContextType {
    ...
    currentView: CurrentView; contentView: 'articles' | 'feedsGrid' | 'inactiveFeeds' | 'dump' | 'privacyPolicy' | 'about' | 'help' | 'notes'; readArticleIds: Map<string, number>; readLaterArticleIds: Set<string>; likedVideoIds: Set<string>;
    isSidebarCollapsed: boolean; sidebarTab: 'yt' | 'rss'; handleSetSidebarTab: (tab: 'yt' | 'rss') => void; isViewsCollapsed: boolean; isYoutubeFeedsCollapsed: boolean; isRssFeedsCollapsed: boolean; isRedditFeedsCollapsed: boolean; isTagsCollapsed: boolean;
    ...
    isSyncDataModalOpen: boolean; setIsSyncDataModalOpen: React.Dispatch<React.SetStateAction<boolean>>;
    isActionsMenuOpen: boolean; setIsActionsMenuOpen: React.Dispatch<React.SetStateAction<boolean>>;
    ...
    translateDigest: (digest: DetailedDigest | ThematicDigest, type: 'detailed' | 'thematic', targetLanguage: string) => Promise<DetailedDigest | ThematicDigest>; isRecommendationsModalOpen: boolean; setIsRecommendationsModalOpen: React.Dispatch<React.SetStateAction<boolean>>;
    ...
    youtubeUnreadTagCounts: Record<string, number>; rssUnreadTagCounts: Record<string, number>;
    unreadPublishedTodayYtCount: number; unreadPublishedTodayRssCount: number; unreadPublishedTodayCount: number;
    unreadReadLaterYtCount: number; unreadReadLaterRssCount: number; unreadReadLaterCount: number;
    historyYtCount: number; historyRssCount: number; historyCount: number;
    unreadFavoritesYtCount: number; unreadFavoritesRssCount: number; unreadFavoritesCount: number;
    ...
    feedsByTag: Map<string, Feed[]>; articlesToShow: Article[]; articlesForNavigation: Article[]; availableTagsForFilter: string[]; unreadVisibleCount: number; headerTitle: string; emptyMessage: string; articleNavigation: { hasNextArticle: boolean; hasPreviousArticle: boolean; onNextArticle: () => void; onPreviousArticle: () => void; };
    ...
    endDemo: () => void;
    handleDemoNext: () => void;
    handleViewChange: (type: string, value?: any) => void; handleSelectFeed: (feedId: string) => void; handleAddFeed: (rawUrl: string, options?: { headless?: boolean; silent?: boolean; }) => Promise<Feed | void>;
    ...
    handleShareToCloudLink: (options?: { tag?: string }) => Promise<{ fullUrl: string; shortCode: string; }>;
    ...
    initialNoteContent: { title: string, content: string, sourceArticleIds: { feedId: string, articleId: string }[] } | null;
    handleOpenNoteEditor: (note?: Note, initialContent?: { title: string, content: string, sourceArticleIds: { feedId: string, articleId: string }[] }) => void;
    handleCloseNoteEditor: () => void;
    handleSaveNote: (note: Omit<Note, 'id' | 'createdAt' | 'updatedAt'>, id?: string) => Promise<Note>;
...
export const AppProvider: FC<{children: ReactNode}> = ({ children }) => {
    ...
    const [articleTags, setArticleTags] = useState<Map<string, string[]>>(() => getStoredData(ARTICLE_TAGS_KEY, new Map(), (k, v) => k === "" ? new Map(Array.isArray(v) ? v : undefined) : v));
    const [readLaterOrderYt, setReadLaterOrderYt] = useState<string[]>(() => getStoredData(READ_LATER_ORDER_YT_KEY, []));
    const [readLaterOrderRss, setReadLaterOrderRss] = useState<string[]>(() => getStoredData(READ_LATER_ORDER_RSS_KEY, []));
    const [tagOrders, setTagOrders] = useState<Record<string, string[]>>(() => getStoredData(TAG_ORDERS_KEY, {}));
    const [favoritesOrderYt, setFavoritesOrderYt] = useState<string[]>(() => getStoredData(FAVORITES_ORDER_YT_KEY, []));
    const [favoritesOrderRss, setFavoritesOrderRss] = useState<string[]>(() => getStoredData(FAVORITES_ORDER_RSS_KEY, []));
    ...
    const [articleViewMode, setArticleViewMode] = useState<ArticleViewMode>(() => getStoredData(ARTICLE_VIEW_MODE_KEY, 'grid'));
    const [readArticleIds, setReadArticleIds] = useState<Map<string, number>>(getPrunedReadArticleIds);
    const [likedVideoIds, setLikedVideoIds] = useState<Set<string>>(() => getStoredData(LIKED_YOUTUBE_VIDEOS_KEY, new Set(), (k, v) => k === "" ? new Set(Array.isArray(v) ? v : undefined) : v));
                    ...
                    callback: async (tokenResponse) => {
                        ...
                        if (tokenResponse.access_token) {
                            ...
                            try {
                                ...
                                if (!isSilentAuthRef.current) {
                                    setToast({ message: 'Signed in, but failed to fetch user profile.', type: 'error' });
                                }
                            } finally {
                                isSilentAuthRef.current = false;
                            }
    ...
    const [sidebarTab, setSidebarTab] = useState<'yt' | 'rss'>(() => getStoredData(SIDEBAR_TAB_KEY, 'yt'));
    const [sidebarFeedsView, setSidebarFeedsView] = useState<SidebarFeedsView>(() => getStoredData(SIDEBAR_FEEDS_VIEW_KEY, 'list'));
    const [isViewsCollapsed, setIsViewsCollapsed] = useState<boolean>(() => getStoredData(VIEWS_COLLAPSED_KEY, false));
    const [isYoutubeFeedsCollapsed, setIsYoutubeFeedsCollapsed] = useState<boolean>(() => getStoredData(YOUTUBE_FEEDS_COLLAPSED_KEY, true));
    const [isYoutubePlaylistsCollapsed, setIsYoutubePlaylistsCollapsed] = useState<boolean>(() => getStoredData(YOUTUBE_PLAYLISTS_COLLAPSED_KEY, true));
    const [isRedditFeedsCollapsed, setIsRedditFeedsCollapse] = useState<boolean>(() => getStoredData(REDDIT_FEEDS_COLLAPSED_KEY, true));
    const [isRssFeedsCollapsed, setIsRssFeedsCollapse] = useState<boolean>(() => getStoredData(RSS_FEEDS_COLLAPSED_KEY, true));
    const [isTagsCollapsed, setIsTagsCollapsed] = useState<boolean>(() => getStoredData(TAGS_COLLAPSED_KEY, true));
    const [isYoutubeTagsCollapsed, setIsYoutubeTagsCollapsed] = useState<boolean>(() => getStoredData(YOUTUBE_TAGS_COLLAPSED_KEY, true));
    const [isRssTagsCollapsed, setIsRssTagsCollapsed] = useState<boolean>(() => getStoredData(RSS_TAGS_COLLAPSED_KEY, true));
    const [expandedTags, setExpandedTags] = useState<Set<string>>(() => getStoredData(EXPANDED_TAGS_KEY, new Set(), (k, v) => k === "" ? new Set(Array.isArray(v) ? v : undefined) : v));
    const [expandedViews, setExpandedViews] = useState<Set<string>>(() => getStoredData(EXPANDED_VIEWS_KEY, new Set(), (k, v) => k === "" ? new Set(Array.isArray(v) ? v : undefined) : v));
    ...
    const [disabledProxies, setDisabledProxies] = useState<Set<string>>(() => getStoredData(DISABLED_PROXIES_KEY, new Set(), (k, v) => k === "" ? new Set(Array.isArray(v) ? v : undefined) : v));
    ...
    const [digestState, setDigestState] = useState<DigestState>({ digest: null, type: null, error: null, isLoading: false, loadingMessage: null });
    ...
    const [autoUploadAfterRefresh, setAutoUploadAfterRefresh] = useState<boolean>(() => getStoredData(AUTO_UPLOAD_AFTER_REFRESH_KEY, false));
    const [autoSummarizeOnRefresh, setAutoSummarizeOnRefresh] = useState<boolean>(() => getStoredData(AUTO_SUMMARIZE_ON_REFRESH_KEY, false));
    const [enableRssAndReddit, setEnableRssAndReddit] = useState<boolean>(() => getStoredData(ENABLE_RSS_REDDIT_KEY, true));
    const [defaultAiLanguage, setDefaultAiLanguage] = useState<string>(() => getStoredData(DEFAULT_AI_LANGUAGE_KEY, 'original'));
    ...
    const [noteFolders, setNoteFolders] = useState<NoteFolder[]>(() => getStoredData(NOTE_FOLDERS_KEY, []));
    const [isNoteEditorModalOpen, setIsNoteEditorModalOpen] = useState(false);
    const [noteToEdit, setNoteToEdit] = useState<Note | null>(null);
    const [initialNoteContent, setInitialNoteContent] = useState<{ title: string, content: string, sourceArticleIds: { feedId: string, articleId: string }[] } | null>(null);
    const [isNotesCollapsed, setIsNotesCollapsed] = useState<boolean>(() => getStoredData(NOTES_COLLAPSED_KEY, true));
    ...
    const [epubDefaults, setEpubDefaults] = useState<{ title: string; filename: string; source: 'selection' | 'view' } | null>(null);
    const [trendingKeywords, setTrendingKeywords] = useState<Array<{ keyword: string; count: number }>>(() => {
    ...
    const [isAiDisabled, setIsAiDisabled] = useState<boolean>(() => getStoredData(AI_DISABLED_KEY, false));
    useEffect(() => {
    ...
    const handleGenerateTrendingKeywords = useCallback(async (forceRefresh = false) => {
        ...
        try {
            ...
            const stopWords = new Set([
                ...
                'new', 'video', 'youtube', 'rss', 'feed', 'post', 'posts', 'article', 'articles', 'channel', 'channels',
                'review', 'update', 'updates', 'watch', 'read', 'official', 'latest', 'news', 'daily', 'weekly', 'monthly',
                'part', 'episode',
    ...
    const {
        ...
        historyYtCount, historyRssCount,
        unreadFavoritesYtCount, unreadFavoritesRssCount,
        ...
        let ptYt = 0, ptRss = 0, rlYt = 0, rlRss = 0, hYt = 0, hRss = 0, fYt = 0, fRss = 0;
        ...
        for (const article of unreadArticles) {
            ...
            if (article.pubDateTimestamp && article.pubDateTimestamp >= todayTimestamp) {
                if (isYtFeed) ptYt++; else ptRss++;
            }
            if (readLaterArticleIds.has(article.id)) {
                if (isYtFeed) rlYt++; else rlRss++;
            }
            if (favoriteFeedIds.has(article.feedId)) {
                if (isYtFeed) fYt++; else fRss++;
            }
        ...
        return {
            unreadPublishedTodayYtCount: ptYt, unreadPublishedTodayRssCount: ptRss,
            unreadReadLaterYtCount: rlYt, unreadReadLaterRssCount: rlRss,
            historyYtCount: hYt, historyRssCount: hRss,
            unreadFavoritesYtCount: fYt, unreadFavoritesRssCount: fRss,
    ...
    const contentView: 'articles' | 'feedsGrid' | 'inactiveFeeds' | 'dump' | 'privacyPolicy' | 'about' | 'help' | 'notes' = useMemo(() => {
    ...
    const youtubeFeeds = useMemo(() => sortedFeeds.filter(feed => feed.url.toLowerCase().includes('youtube.com')), [sortedFeeds]);
    const rssAndOtherFeeds = useMemo(() => sortedFeeds.filter(feed => !feed.url.toLowerCase().includes('youtube.com')), [sortedFeeds]);
        ...
        switch (currentView.type) {
            ...
            case 'search':
                if (!currentView.value) {
                    ...
                    rawArticles = articlesToSearch.filter(a => a.title.toLowerCase().includes(query) || a.description.toLowerCase().includes(query));
                }
    ...
    }, [currentView, feedsById, allArticles, readLaterArticleIds, readArticleIds, articlesById, favoriteFeeds, activeTagFilter, readLaterOrderYt, readLaterOrderRss, tagOrders, favoritesOrderYt, favoritesOrderRss, sidebarTab]);
    ...
    const headerTitle = useMemo(() => {
        switch (currentView.type) {
            ...
            case 'keyword-articles': return `Keyword: "${currentView.value}"`;
            ...
            case 'ai-summary-yt': return 'YT with AI Summary';
    ...
    const emptyMessage = useMemo(() => {
        ...
        switch (currentView.type) {
            ...
            case 'keyword-articles': return 'No articles found for this keyword.';
            ...
            case 'note-folder': return 'This folder is empty. Save summaries from articles to create notes.';
            case 'published-today': return 'No articles published today.';
            case 'ai-summary-yt': return 'No YouTube articles with an AI summary found.';
    ...
    const canArticleZoomOut = useMemo(() => ZOOM_LEVELS.indexOf(articleZoomLevel) < ZOOM_LEVELS.length - 1, [articleZoomLevel]);

    const handleViewChange = useCallback((type: string, value?: any) => {
    ...
    const handleImportData = useCallback((data: SyncData, options?: { silent?: boolean }) => {
        try {
            ...
            data.feeds.forEach((importedFeed: Omit<Feed, 'id' | 'items' | 'error'>) => {
                ...
                const articlesFromBackup = (data.articlesByFeedUrl?.[importedFeed.url] || []).map((art: Article) => {
                    const content = isYouTubeFeed && art.description ? art.description : (art.content ?? '');
                    return {
                        ...art,
                        content: content,
        ...
        setReadLaterOrderYt, setReadLaterOrderRss, setTagOrders, setFavoritesOrderYt, setFavoritesOrderRss,
        setGridZoomLevel, setArticleZoomLevel, setAutoplayMode, setToast,
        currentView, handleViewChange, setNotes, setNoteFolders
    ]);
    ...
    const safeSetLocalStorage = useCallback((key: string, value: any) => {
        const stringify = (v: any) => JSON.stringify(v, (_, val) => {
        ...
        try {
            ...
            try {
                ...
                const isStillQuotaError = retryError.name === 'QuotaExceededError' || (retryError.code && (retryError.code === 22 || retryError.code === 1014));
                ...
                articlesToConsiderForPruning.sort((a, b) => (a.pubDateTimestamp || Infinity) - (b.pubDateTimestamp || Infinity));

                const articlesToRemoveCount = Math.ceil(articlesToConsiderForPruning.length * 0.25);
                const articlesToRemoveIds = new Set(articlesToConsiderForPruning.slice(0, articlesToRemoveCount).map(a => a.id));
        ...
        try {
            ...
            updatedFeeds.forEach(feed => {
                if (feed.isPlaylist) {
                    ...
                    const unreadItems = feed.items.filter((item: Article) => !readArticleIdsRef.current.has(item.id));
    ...
    const handleConfirmAddChannel = useCallback(async (channelUrl: string, channelTitle: string) => {
        ...
        try {
            const newFeed = await fetchAndParseRss(channelUrl, channelTitle, handleProxyAttempt, disabledProxies, proxyStats, 500);
        ...
        try {
            ...
            if (existingFeed) {
                ...
                const newFeed: Feed = {
                    ...
                    items: [videoArticle], error: null, iconUrl: videoArticle.imageUrl
                };
    ...
    const handleShareToCloudLink = useCallback(async (options?: { tag?: string }): Promise<{ fullUrl: string; shortCode: string; }> => {
        ...
        const uploadChunk = async (chunk: string, title: string, forceService?: 'dpaste' | 'pastegg'): Promise<{ service: 'dpaste' | 'pastegg', id: string }> => {
            const allServices = [
                {
                    ...
                    upload: async (proxy: typeof PROXIES[0]) => {
                        ...
                        const response = await fetch(proxyUrl, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body });
                        if (!response.ok) {
                            const errorText = await response.text();
                            throw new Error(`Service dpaste.org via ${proxy.name} responded with status ${response.status}: ${errorText}`);
                        }
                ...
                },
                {
                    ...
                    upload: async (proxy: typeof PROXIES[0]) => {
                        ...
                        const body = JSON.stringify({
                            ...
                            files: [{ name: "media-feeder-backup.txt", content: { format: "text", value: chunk } }]
                        });
                        const response = await fetch(proxyUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body });
                        if (!response.ok) {
                            const errorText = await response.text();
                            throw new Error(`Service paste.gg via ${proxy.name} responded with status ${response.status}: ${errorText}`);
                        }
            ...
            for (const service of servicesToTry) {
                for (const proxy of PROXIES) {
                    try {
                        ...
                        console.warn(`Sharing with ${service.name} via ${proxy.name} failed:`, lastError.message);
                    }
                }
            }
        ...
        try {
            ...
            const remainingPastePromises = remainingChunks.map(async (chunk, index) => {
                setToast({ message: `Uploading part ${index + 2} of ${totalChunks}...`, type: 'success' });
                return await uploadChunk(chunk, `Media-Feeder Multipart Backup ${shareId} (${index + 2}/${totalChunks})`, chosenService);
            });
    ...
    const handleAddFeed = useCallback(async (rawUrl: string, options?: { headless?: boolean, silent?: boolean }): Promise<Feed | void> => {
        ...
        try {
            ...
            if (multiPartMatch) {
                try {
                    ...
                    const fetchPromises = ids.map(async (id) => {
                        if (serviceName === 'dpaste') {
                            ...
                            if (metadata.status !== 'success') throw new Error(`Could not find paste.gg metadata for chunk: ${id}. It may have expired.`);
                            ...
                            if (typeof content !== 'string') throw new Error(`Could not extract content from the paste.gg file data.`);
                            return content;
                        } else {
                    ...
                    for (const content of chunkContents) {
                        const match = content.match(/^media-feeder-multipart:v1:(\d+):(\d+):(\w+):([\s\S]+)$/);
                        if (!match) {
                        ...
                        if (shareId !== id || totalChunks !== total) {
                            throw new Error("Mismatched backup parts. Ensure all parts are from the same share link.");
                        }
                        chunks[index] = data;
                    }
                    if (Object.keys(chunks).length !== totalChunks) {
                        throw new Error(`Missing backup parts. Expected ${totalChunks}, but found ${Object.keys(chunks).length}.`);
                    }
                ...
                try {
                    const service = serviceCodeMatch[1] || (serviceCodeMatch[2] === 'p' ? 'pastegg' : 'dpaste');
                    ...
                    if (service === 'dpaste') {
                        ...
                        const metadataContent = await fetchViaProxy(metadataUrl, 'rss');
                        const metadata = JSON.parse(metadataContent);
                        if (metadata.status !== 'success') throw new Error(`Could not find paste.gg metadata for the provided code. It may have expired.`);
                        const fileId = metadata?.result?.files?.[0]?.id;
    ...
    const handleCloseArticleModal = useCallback(() => {
        ...
        if (backgroundView.value) {
            ...
            } else if (typeof backgroundView.value === 'object' && backgroundView.value.name && backgroundView.value.type) {
    ...
    const articlesForNavigation = useMemo(() => {
        ...
        const sortArticles = (articles: Article[]): Article[] => {
            ...
            if (viewForFiltering.type === 'feed' && articles.length > 0 && articles.some(a => a.order !== undefined)) {
                return [...articles].sort((a, b) => (a.order ?? Infinity) - (b.order ?? Infinity));
            }
        ...
        const processedArticles = sortArticles(needsDeduplication ? deduplicateArticles(tabFilteredArticles) : tabFilteredArticles);
                            ...
                            setSelectedArticle(article);
                             ...
                             try {
                                ...
                                if (isLikelyVideo) {
                                    ...
                                    setToast({ message: 'Could not find the shared article in your feeds.', type: 'error' });
                                    window.location.hash = '#/all-subscriptions';
                                }
                            } catch (e) {
                                setToast({ message: `Could not load shared article: ${e instanceof Error ? e.message : 'Unknown error.'}`, type: 'error' });
                                window.location.hash = '#/all-subscriptions';
                            }
                        }
                    } catch (e) {
    ...
    const handleQuotaError = useCallback((options?: { source: 'auto' | 'manual' }) => {
        setToast({
            message: "Gemini API quota exceeded. Please check your API key's billing status or try again later.",
            type: 'error',
        });
    ...
    const runInBackgroundSummaryGeneration = useCallback(async (newArticles: Article[], isBroadRefresh = false) => {
        console.debug('[AutoSummary] Starting background summary generation check.');
        if (!autoSummarizeOnRefresh || newArticles.length === 0) {
            if (!autoSummarizeOnRefresh) console.debug('[AutoSummary] Aborting: Auto-summarize is disabled in settings.');
        ...
        for (const article of videosToSummarize) {
            ...
            try {
                console.debug(`[AutoSummary] Checking for transcripts for video ID ${videoId}.`);
                const choices = await fetchAvailableCaptionChoices(videoId);

                if (choices.length > 0) {
                    console.debug(`[AutoSummary] Found ${choices.length} transcript choice(s). Attempting to use the first one.`);
                    const transcriptLines = await fetchAndParseTranscript(choices[0].url);
                    if (transcriptLines.length > 3) {
                        console.debug(`[AutoSummary] Transcript is long enough (${transcriptLines.length} lines). Generating structured summary.`);
                        ...
                        console.debug(`[AutoSummary] Successfully generated structured summary for "${article.title}".`);
                        continue;
                    } else {
                        console.debug(`[AutoSummary] Transcript found but too short (${transcriptLines.length} lines). Falling back to text-based summary.`);
                    }
                } else {
                ...
                if (textForSummary.length > 100) {
                    console.debug(`[AutoSummary] Text content is long enough (${textForSummary.length} chars). Generating text summary.`);
                    const { summary, sources } = await summarizeText(textForSummary, article.link, aiModel, defaultAiLanguage, 'video');
                    handleSummaryGenerated(article.id, summary, sources);
                    console.debug(`[AutoSummary] Successfully generated text-based summary for "${article.title}".`);
                } else {
                    console.debug(`[AutoSummary] Text content is too short (${textForSummary.length} chars). Skipping summary.`);
                }
            } catch (error) {
        ...
        console.debug('[AutoSummary] Finished background summary generation process.');
