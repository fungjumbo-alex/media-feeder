  ...
  for (const proxy of IMAGE_PROXIES) {
    try {
      ...
      if (!contentType || !contentType.startsWith('image/')) {
        throw new Error(
          `Response from ${proxy.name} is not an image. Content-Type: ${contentType}`
        );
      }

      const blob = await response.blob();
      if (blob.size < 100) {
        // Reject tiny files which are likely errors/trackers
        throw new Error(
          `Proxy ${proxy.name} returned a tiny file (${blob.size} bytes), likely an error page or tracking pixel.`
        );
      }
...
export const createEpub = async (articles: Article[], title: string): Promise<Blob> => {
  ...
  manifestItems.push(
    `<item id="${tocId}" href="${tocFileName}" media-type="application/xhtml+xml" />`
  );
  spineItems.push(`<itemref idref="${tocId}" />`);
  contentFolder.file(tocFileName, getTocXhtml(title, articles));
  tocNavPoints.push(
    `<navPoint id="navpoint-toc" playOrder="1"><navLabel><text>Table of Contents</text></navLabel><content src="${tocFileName}"/></navPoint>`
  );
  ...
  for (let i = 0; i < articles.length; i++) {
    ...
    if (article.structuredSummary) {
      ...
      if (article.structuredSummary.sections && article.structuredSummary.sections.length > 0) {
        ...
        article.structuredSummary.sections.forEach(section => {
          const timestampLink = article.link
            ? `${article.link}&t=${Math.floor(section.timestamp)}s`
            : '#';
    ...
    const imagePromises = images.map(async img => {
      ...
      try {
        ...
        manifestItems.push(
          `<item id="${imageId}" href="images/${imageFileName}" media-type="${mimeType}" />`
        );
