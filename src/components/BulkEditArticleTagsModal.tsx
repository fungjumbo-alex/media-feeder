            ...
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg m-4 relative flex flex-col max-h-[90vh]">
                ...
                <div className="flex-shrink-0">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        <TagIcon className="w-6 h-6 text-indigo-400" />Bulk Tag Articles
                    </h2>
                    ...
                    <div>
                        ...
                        <div className="flex flex-col gap-2 bg-gray-900/50 p-2 rounded-lg">
                            <label className="flex items-center gap-2 cursor-pointer p-1 rounded-md hover:bg-gray-700/50 transition-colors">
                                <input type="radio" name="edit-mode" value="add" checked={editMode === 'add'} onChange={() => setEditMode('add')} className="h-4 w-4 text-indigo-600 bg-gray-700 border-gray-500 focus:ring-indigo-500" />
                                <span className="text-sm font-medium">Add Tags</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer p-1 rounded-md hover:bg-gray-700/50 transition-colors">
                                <input type="radio" name="edit-mode" value="set" checked={editMode === 'set'} onChange={() => setEditMode('set')} className="h-4 w-4 text-indigo-600 bg-gray-700 border-gray-500 focus:ring-indigo-500" />
                                <span className="text-sm font-medium">Set (Overwrite) Tags</span>
                            </label>
                        </div>
                         {editMode === 'set' && <p className="mt-2 text-xs text-yellow-400/80">Warning: 'Set' will replace all existing tags on the selected articles.</p>}
                    </div>
                     <div className="flex flex-col flex-grow min-h-0">
                        ...
                        <div className="border border-gray-700 rounded-md p-2 flex-grow overflow-y-auto min-h-[100px]">
                             <div className="flex flex-wrap gap-2">
                                {uniqueTags.length > 0 ? uniqueTags.map(tag => <TagButton key={tag} tag={tag} />) : <span className="text-xs text-gray-500 p-2">No existing tags. Add one above.</span>}
                            </div>
                        </div>
                    </div>
                </div>
