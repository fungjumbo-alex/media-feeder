...
const ErrorDisplay: React.FC<{ message: string; onRetry: () => void }> = ({ message, onRetry }) => (
    <div className="flex flex-col justify-center items-center h-full">
        <div className="bg-red-500/10 text-red-400 p-4 rounded-lg m-4 text-center">
            ...
            <p>{message}</p>
            {message.toLowerCase().includes('permission') && <p className="mt-1 text-xs">The app may not have sufficient permissions. Please try authenticating again.</p>}
        </div>
        <button onClick={onRetry} className="mt-4 px-4 py-2 rounded-md bg-indigo-600 text-white font-semibold hover:bg-indigo-500">
...
export const ImportYouTubeModal: React.FC = () => {
    ...
    const channelsToShow = useMemo(() => {
        return subscriptions.filter((sub: YouTubeSubscription) => !existingChannelIds.has(sub.channelId));
    ...
    const handleConfirmAdd = async () => {
        const selectedSubs = subscriptions.filter((sub: YouTubeSubscription) => selectedSubscriptionIds.has(sub.id));
    ...
    const renderContent = () => {
        switch (status) {
            ...
            case 'error':
                return <ErrorDisplay message={error || "An unknown error occurred."} onRetry={handleFetchYouTubeSubscriptions} />;
            case 'loaded':
                if (channelsToShow.length === 0) {
                    return <div className="text-center text-gray-400 p-8">All your YouTube subscriptions are already in the app!</div>;
                }
                return (
                        ...
                        <div className="flex-grow overflow-y-auto">
                            {channelsToShow.map((sub: YouTubeSubscription) => (
                                ...
                                >
                                    ...
                                    />
                                    <img src={sub.thumbnailUrl} alt={sub.title} className="w-10 h-10 rounded-full bg-gray-700" />
                ...
                return (
                    <div className="text-center flex flex-col items-center justify-center h-full px-4">
                        ...
                        <p className="text-gray-400 mb-6 max-w-lg">
                            Fetch your full list of subscriptions from YouTube and add them to your reader in one go.
                        </p>
                        <button onClick={handleFetchYouTubeSubscriptions} className="px-6 py-3 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-500">
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl h-[90vh] m-4 flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white flex items-center">
                        <YouTubeIcon className="w-6 h-6 mr-3 text-red-500" />Import from YouTube
                    </h2>
