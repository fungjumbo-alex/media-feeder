                        ...
                        {userProfile ? (
                             <>
                                {driveSyncStatus.status === 'no_permission' && (
                                    <>
                                        <p className="text-sm text-yellow-400 mb-4">This app needs permission to access Google Drive for syncing. Please grant access to continue.</p>
                                        <ActionButton onClick={() => handleGoogleSignIn({ showConsentPrompt: true })} className="bg-indigo-600 hover:bg-indigo-500 text-white">
                                            Grant Drive Access
                                        </ActionButton>
                                    </>
                                )}
                                {driveSyncStatus.status === 'ready' && (
                                    <>
                                        <p className="text-sm text-gray-400 mb-4">Sync your data across devices using your Google Drive. A file named <strong>media-feeder-data-v2.json</strong> will be created in your "My Drive" folder.</p>
                                        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            <ActionButton onClick={handleUploadClick} disabled={isUploading} className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-400/50 text-white">
                                                {isUploading ? 'Uploading...' : 'Upload to Drive'}
                                            </ActionButton>
                                            <ActionButton onClick={handleDownloadClick} disabled={isDownloading} className="bg-green-600 hover:bg-green-500 disabled:bg-green-400/50 text-white">
                                                {isDownloading ? 'Downloading...' : 'Download from Drive'}
                                            </ActionButton>
                                        </div>
                                        {driveSyncStatus.fileMetadata?.modifiedTime && (
                                            <p className="mt-4 text-center text-xs text-gray-500">
                                                Last sync: {formatRelativeDate(new Date(driveSyncStatus.fileMetadata.modifiedTime).getTime())}
                                            </p>
                                        )}
                                    </>
                                )}
                                {driveSyncStatus.status === 'checking' && <p className="text-sm text-gray-400">Checking Drive status...</p>}
                                {driveSyncStatus.status === 'error' && <p className="text-sm text-red-400">{driveSyncStatus.error}</p>}
                                {error && <p className="mt-2 text-sm text-red-400">{error}</p>}
                            </>
                        ) : (
                            <>
                                <p className="text-sm text-gray-400 mb-4">Sign in with your Google Account to enable syncing your feeds and settings with Google Drive.</p>
                                <ActionButton onClick={() => handleGoogleSignIn()} className="bg-white/90 hover:bg-white text-gray-800">
