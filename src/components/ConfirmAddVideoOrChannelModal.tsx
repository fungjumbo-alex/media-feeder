    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg m-4 relative">
        ...
        <p className="text-gray-400 mb-6">
          You've provided a link to a single video. What would you like to add?
        </p>
        ...
        <div className="bg-gray-700/50 p-4 rounded-lg flex items-start gap-4 mb-6">
          <img
            src={video.imageUrl || ''}
            alt={video.title}
            className="w-32 h-18 object-cover rounded bg-gray-700 flex-shrink-0"
          />
          <div className="flex-grow min-w-0">
            <p className="text-sm text-gray-400">Video:</p>
            <p className="font-semibold text-gray-100 truncate" title={video.title}>
              {video.title}
            </p>
            <p className="text-sm text-gray-400 mt-2">From Channel:</p>
            <p className="font-semibold text-gray-100 truncate" title={video.feedTitle}>
              {video.feedTitle}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          >
            ...
            <p className="text-sm text-gray-400 mt-1">Adds only this video to your library.</p>
          </button>
          <button
        ...
        <div className="mt-6 flex justify-end">
          <button
            type="button"
            onClick={onClose}
            className="px-4 py-2 rounded-md text-gray-300 bg-gray-700/50 hover:bg-gray-600"
          >
            Cancel
          </button>
        </div>
