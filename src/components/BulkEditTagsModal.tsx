...
export const BulkEditTagsModal: React.FC = () => {
    const { bulkEditModalConfig, setBulkEditModalConfig, feeds, allTags, handleBulkUpdateTags, handleClearAllTags } = useAppContext();
    ...
    useEffect(() => {
        ...
        setSelectedFeedIds(currentSelected => {
            ...
            currentSelected.forEach(id => { if (visibleFeedIds.has(id)) newSelection.add(id); });
            return newSelection;
        });
    }, [filteredFeeds]);
    ...
    const areAllVisibleSelected = filteredFeeds.length > 0 && selectedFeedIds.size === filteredFeeds.length;
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl h-[90vh] m-4 flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white flex items-center"><ModalIcon className="w-6 h-6 mr-3 text-indigo-400" />{modalTitle}</h2>
                    ...
                    <div className="w-full md:w-1/2 flex flex-col">
                        ...
                        <div className="flex-shrink-0 mb-4">
                            <div className="flex items-center gap-3">
                                <input id="filter-feeds" type="checkbox" checked={applyPrimaryFilter} onChange={() => setApplyPrimaryFilter(p => !p)} className="h-4 w-4 rounded border-gray-500 bg-gray-700 text-indigo-600 focus:ring-indigo-500" />
                                <label htmlFor="filter-feeds" className="text-sm text-gray-300">{primaryFilterLabel}</label>
                            </div>
                        </div>
                        <div className="border border-gray-700 rounded-md flex-grow flex flex-col min-h-0">
                            <div className="flex items-center gap-3 px-3 py-2 bg-gray-900/50 border-b border-gray-700 flex-shrink-0">
                                <input id="select-all" type="checkbox" checked={areAllVisibleSelected} onChange={handleSelectAll} className="h-4 w-4 rounded border-gray-500 bg-gray-700 text-indigo-600 focus:ring-indigo-500" />
                                <label htmlFor="select-all" className="text-sm font-semibold text-gray-300">Select All ({selectedFeedIds.size} / {filteredFeeds.length})</label>
                            </div>
                                ...
                                {filteredFeeds.length > 0 ? filteredFeeds.map(feed => {
                                    ...
                                    return (
                                        <div key={feed.id} className="flex items-center gap-3 p-2 rounded-md hover:bg-gray-700/50">
                                            ...
                                            <label htmlFor={`feed-${feed.id}`} className="text-sm text-gray-200 truncate flex-grow cursor-pointer" title={feed.title}>{feed.title}</label>
                                        </div>
                                    )
                        ...
                        {mode !== 'favorite' ? (
                            <>
                                <div className="flex flex-col gap-4 flex-grow min-h-0">
                                    <div>
                                        ...
                                        <div className="flex flex-col gap-2 bg-gray-900/50 p-2 rounded-lg">
                                            <label className="flex items-center gap-2 cursor-pointer p-1 rounded-md hover:bg-gray-700/50 transition-colors"><input type="radio" name="edit-mode" value="add" checked={localEditMode === 'add'} onChange={() => setLocalEditMode('add')} className="h-4 w-4 text-indigo-600 bg-gray-700 border-gray-500 focus:ring-indigo-500" /><span className="text-sm font-medium">Add Tags</span></label>
                                            <label className="flex items-center gap-2 cursor-pointer p-1 rounded-md hover:bg-gray-700/50 transition-colors"><input type="radio" name="edit-mode" value="set" checked={localEditMode === 'set'} onChange={() => setLocalEditMode('set')} className="h-4 w-4 text-indigo-600 bg-gray-700 border-gray-500 focus:ring-indigo-500" /><span className="text-sm font-medium">Set (Overwrite) Tags</span></label>
                                        </div>
                                    </div>
                                    <div className="flex flex-col flex-grow min-h-0">
                                        ...
                                        <form onSubmit={handleAddNewTag} className="flex gap-2 mb-3">
                                            ...
                                            />
                                            <button type="submit" className="px-3 py-1 rounded-md text-white bg-gray-600 hover:bg-gray-500 font-semibold text-sm">Add</button>
                                        </form>
                                        <div className="border border-gray-700 rounded-md p-2 flex-grow overflow-y-auto">
                                             <div className="flex flex-wrap gap-2">
                                                ...
                                                )) : <span className="text-xs text-gray-500 p-2">No existing tags. Add one above.</span>}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div className="bg-gray-700/50 p-3 rounded-md text-sm text-gray-400 flex-shrink-0">
                                    ...
                                    <p>You will <strong className="text-indigo-400">{localEditMode === 'add' ? 'ADD' : 'SET'}</strong> {tagsToApply.size > 0 ? <>the tag(s) <strong className="text-indigo-400">{Array.from(tagsToApply).join(', ')}</strong></> : 'tags'} to <strong className="text-indigo-400">{selectedFeedIds.size}</strong> selected feed(s).</p>
                                    {localEditMode === 'set' && <p className="mt-1 text-yellow-400/80">Warning: 'Set' will replace all existing tags on the selected feeds.</p>}
                                </div>
                            </>
                        ) : (
                            <div className="bg-gray-700/50 p-3 rounded-md text-sm text-gray-400 h-full flex items-center justify-center">
                                <p className="text-center">You are about to <strong className="text-yellow-400">FAVORITE</strong> <strong className="text-yellow-400">{selectedFeedIds.size}</strong> selected feed(s).</p>
                            </div>
                        )}
                    </div>
                </div>
                <footer className="p-4 border-t border-gray-700 flex-shrink-0 bg-gray-800/80 flex justify-between items-center">
                    ...
                    <div className="flex gap-4">
                        ...
                        <button type="button" onClick={handleSubmit} disabled={selectedFeedIds.size === 0 || (mode !== 'favorite' && tagsToApply.size === 0)} className="px-6 py-2 rounded-md bg-indigo-600 text-white font-semibold hover:bg-indigo-500 disabled:bg-indigo-400/50 disabled:cursor-not-allowed">Apply Changes</button>
                    </div>
                </footer>
            </div>
        </div>
