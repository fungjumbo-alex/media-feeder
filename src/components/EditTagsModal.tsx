...
export const EditTagsModal: React.FC = () => {
    const { feedToEditTags, setFeedToEditTags, handleSaveFeedTags, allTags } = useAppContext();
    
    const [selectedTags, setSelectedTags] = useState<Set<string>>(new Set());
    const [newTagInput, setNewTagInput] = useState('');

    const isOpen = !!feedToEditTags;
    const feed = feedToEditTags;
    ...
    const newCustomTags = useMemo(() => {
        ...
        return Array.from(selectedTags).filter(tag => !existingTagsSet.has(tag)).sort();
    ...
    const handleConfirm = () => {
        if (!feed) return;
        handleSaveFeedTags(feed.id, Array.from(selectedTags).sort());
        onClose();
    };

    const onClose = () => setFeedToEditTags(null);
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-lg m-4 relative flex flex-col max-h-[90vh]">
                ...
                <div className="flex-shrink-0">
                    <h2 className="text-xl font-bold text-white flex items-center gap-2">
                        <TagIcon className="w-6 h-6 text-indigo-400" />Edit Tags
                    </h2>
                    <p className="text-gray-400 mt-1 mb-6 truncate" title={feed.title}>
                        For: <span className="font-semibold text-gray-300">{feed.title}</span>
                    </p>
                </div>
                <div className="flex-grow overflow-y-auto pr-2 -mr-2">
                    {feed.description && (
                        <div className="mb-4 p-3 bg-gray-900/50 rounded-md max-h-24 overflow-y-auto">
                        ...
                        {newCustomTags.length > 0 && (
                            <div className="mb-6">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {newCustomTags.map(tag => <TagButton key={tag} tag={tag} />)}
                                </div>
                            </div>
                        )}
                        {allTags.length > 0 && (
                         ...
                         <div>
                            ...
                            <form onSubmit={handleAddNewTag} className="flex gap-2">
                                ...
                                />
                                <button type="submit" className="px-4 py-2 rounded-md text-white bg-gray-600 hover:bg-gray-500 font-semibold">
                                    Add
                                </button>
                            </form>
                        </div>
