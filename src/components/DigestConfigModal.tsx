...
export const DigestConfigModal: React.FC = () => {
  ...
  const handleSelectAll = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.checked) {
      setSelectedArticleIds(
        new Set(eligibleArticles.slice(0, DETAILED_DIGEST_LIMIT).map(a => a.id))
      );
    } else {
  ...
  const isAllSelected =
    ...
    selectedArticleIds.size === Math.min(eligibleArticles.length, DETAILED_DIGEST_LIMIT);

  const renderContent = () => {
    if (isCheckingTranscripts) {
      return (
        <LoadingState
          message={`Checking ${articlesForDigest.length} items for summarizable content...`}
        />
      );
    }
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6 flex-1 min-h-0">
            ...
            {!isThematicMode && (
              <div className="flex items-center gap-3 px-3 py-2 bg-gray-900/50 border-b border-gray-700 flex-shrink-0">
                ...
                <label htmlFor="select-all-digest" className="text-sm font-semibold text-gray-300">
                  Select All ({selectedArticleIds.size} /{' '}
                  {Math.min(eligibleArticles.length, DETAILED_DIGEST_LIMIT)})
                </label>
              </div>
            )}
              ...
              {(isThematicMode ? articlesForDigest : eligibleArticles).map(article => (
                ...
                >
                  ...
                  <img
                    ...
                    alt=""
                    className="w-10 h-6 object-cover rounded bg-gray-700 flex-shrink-0"
                  />
                  <label
        ...
        <div className="w-full md:w-1/3">
          ...
          <p className="text-xs text-gray-400 mb-3">
            Choose the language for the final summary. "Original" will use the content's source
            language.
          </p>
