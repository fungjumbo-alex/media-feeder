    ...
    >
      ...
      {zoomLevel !== 'xl' && (
        <div className="p-4 flex flex-col flex-grow">
          <h3
            className={`font-bold text-gray-100 mb-2 leading-tight flex-grow ${titleSizeClasses[zoomLevel]}`}
          >
            {article.title}
          </h3>
          {article.tags && article.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mb-2">
              {article.tags.map(tag => (
                <span
                  key={tag}
                  className="px-1.5 py-0.5 text-xs font-medium bg-gray-700 text-gray-300 rounded-full"
                >
                  {tag}
                </span>
              ))}
            ...
            <div className="flex items-center gap-1 flex-shrink-0 ml-2">
              ...
              >
                <div className="relative h-5 w-5">
                  ...
                  {isSelected && (
                    <svg
                      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 h-4 w-4 text-white pointer-events-none"
                      viewBox="0 0 16 16"
                      fill="currentColor"
                    >
