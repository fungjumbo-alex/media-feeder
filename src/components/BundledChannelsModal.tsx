...
const bundles = [
    { name: 'Science', data: scienceBundle.feeds as Omit<Feed, 'id' | 'items' | 'error'>[], description: 'Feeds related to science and discovery.' },
    { name: 'Tech', data: techBundle.feeds as Omit<Feed, 'id' | 'items' | 'error'>[], description: 'General technology news and analysis.' },
    { name: 'Gadgets', data: gadgetsBundle.feeds as Omit<Feed, 'id' | 'items' | 'error'>[], description: 'Reviews and news about consumer electronics.' },
];

export const BundledChannelsModal: React.FC = () => {
    const { isBundledChannelsModalOpen, setIsBundledChannelsModalOpen, handleImportBundledChannels, feeds } = useAppContext();
    ...
    const handleImportClick = (bundleName: string, feedsToImport: Omit<Feed, 'id' | 'items' | 'error'>[]) => {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl h-[90vh] m-4 flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="p-4 border-b border-gray-700 flex-shrink-0 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-white flex items-center"><PackageIcon className="w-6 h-6 mr-3 text-indigo-400" />Add Channel Bundles</h2>
                    <button onClick={onClose} className="text-gray-400 hover:text-white"><XIcon className="w-6 h-6" /></button>
                </header>
                <div className="p-6 flex-1 overflow-y-auto">
                    <p className="text-gray-400 mb-6">
                        Quickly add a set of curated channels based on a topic. The app will skip any channels you already subscribe to.
                    </p>
                    <div className="space-y-4">
                        {bundles.map(bundle => {
                            ...
                            return (
                                <div key={bundle.name} className="bg-gray-700/50 rounded-lg p-4 flex flex-col sm:flex-row gap-4 items-start">
                                    <div className="flex-grow">
                                        ...
                                        <p className="text-xs text-gray-500 mt-2">Contains {channelsInBundle.length} channels.</p>
                                    </div>
