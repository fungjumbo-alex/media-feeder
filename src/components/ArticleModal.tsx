...
import type { TranscriptLine, CaptionChoice, StructuredVideoSummary, YouTubeComment, AiModel } from '../types';
...
const Tooltip: React.FC<{ text: string; children: React.ReactNode; isMobile?: boolean }> = ({ text, children, isMobile }) => {
    if (isMobile) {
...
export const ArticleModal: React.FC = () => {
    ...
    const [displayStructuredSummary, setDisplayStructuredSummary] = useState<StructuredVideoSummary | null>(null);
    ...
    const isLiked = useMemo(() => videoId ? likedVideoIds.has(videoId) : false, [videoId, likedVideoIds]);
    ...
    const channelLinkUrl = (currentFeed?.isPlaylist && currentFeed.channelUrl) ? currentFeed.channelUrl : article?.feedId;
    ...
    const handleLanguageChange = async (lang: string) => {
        ...
        try {
            if (structuredSummary) {
                const translatedSummary = await translateStructuredSummary(structuredSummary, lang, aiModel as AiModel);
    ...
    const handlePanelResizeStart = (e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
        e.preventDefault(); e.stopPropagation(); setIsPanelResizing(true);
    ...
    const handlePanelResizeMove = useCallback((e: MouseEvent | TouchEvent) => {
        ...
        const minHeight = 40; const maxHeight = window.innerHeight - 80;
        setPanelHeight(Math.max(minHeight, Math.min(newHeight, maxHeight)));
    }, [isPanelResizing]);

    const handlePanelResizeEnd = useCallback(() => {
        setIsPanelResizing(false); panelResizeStartRef.current = null;
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (event.key === ' ' || event.code === 'Space') {
                ...
                isTheaterMode ? setIsTheaterMode(false) : handleClose();
            } else if (event.key === 'ArrowRight' && articleNavigation.hasNextArticle && !isTheaterMode) {
                articleNavigation.onNextArticle();
            } else if (event.key === 'ArrowLeft' && articleNavigation.hasPreviousArticle && !isTheaterMode) {
    ...
    const handleTouchStart = (e: React.TouchEvent) => {
        ...
        touchEndRef.current = null; touchStartRef.current = { x: e.targetTouches[0].clientX, isInsideVideo };
    };
    const handleTouchMove = (e: React.TouchEvent) => { if (touchStartRef.current && !touchStartRef.current.isInsideVideo) touchEndRef.current = e.targetTouches[0].clientX; };
            ...
            if (videoId) {
                ...
                try {
                    ...
                    if (choices.length > 0) {
                        ...
                        if (transcriptLines.length < 3) throw new Error("Transcript is too short to summarize.");

                        const { summary: newStructuredSummary, sources } = await summarizeYouTubeVideo(article.title, transcriptLines, aiModel as AiModel, defaultAiLanguage);
                    ...
                    console.warn("Transcript summarization failed, falling back to text summary. Reason:", transcriptError);
                    ...
                    const { summary: newSummary, sources } = await summarizeText(plainText, article.link, aiModel as AiModel, defaultAiLanguage, 'video');
                ...
                if (plainText.trim().length < 50 && !article.link) {
                    throw new Error("The content from the feed is too short to summarize, and no link is available for external lookup.");
                }
                const { summary: newSummary, sources } = await summarizeText(plainText, article.link, aiModel as AiModel, defaultAiLanguage);
                handleSummaryGenerated(article.id, newSummary, sources);
                handleSuccessfulApiCall();
            }
        } catch (e) {
    ...
    }, [article, videoId, aiModel, handleSummaryGenerated, defaultAiLanguage, handleSuccessfulApiCall, handleQuotaError]);

    useEffect(() => {
        if (videoId) {
            const fetchChoices = async () => {
                setIsFetchingChoices(true); setTranscriptError(null);
                try {
                    const choices = await fetchAvailableCaptionChoices(videoId);
                ...
                } finally { setIsFetchingChoices(false); }
            };
            fetchChoices();
        }
    }, [videoId]);
    ...
    useEffect(() => {
        if (selectedCaptionUrl) {
            const fetchTranscript = async () => {
                setIsFetchingTranscript(true); setTranscriptError(null); setTranscriptData(null);
                try {
                    ...
                    setTranscriptError(e instanceof Error ? e.message : "Failed to load transcript.");
                    setIsSummarizing(false);
                } finally { setIsFetchingTranscript(false); }
            };
            fetchTranscript();
        }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => { handleLikeVideo(article, { isAutoLike: true }); }, autoLikeDelaySeconds * 1000);
        return () => clearTimeout(timer);
    }, [article, videoId, autoLikeYouTubeVideos, isLiked, handleLikeVideo, autoLikeDelaySeconds]);

    const stopTranscriptSync = useCallback(() => { if (timeUpdateIntervalRef.current) clearInterval(timeUpdateIntervalRef.current); timeUpdateIntervalRef.current = null; }, []);
    const startTranscriptSync = useCallback(() => {
        stopTranscriptSync();
        const transcript = transcriptData?.transcript;
        if (!playerRef.current || !transcript) return;
        timeUpdateIntervalRef.current = window.setInterval(() => {
            const localPlayer = playerRef.current; const localTranscript = transcriptData?.transcript;
            if (!localPlayer || !localTranscript) return;
            const currentTime = localPlayer.getCurrentTime();
            if (typeof currentTime !== 'number') return;
            let newIndex = -1;
            for (let i = localTranscript.length - 1; i >= 0; i--) if (localTranscript[i].start <= currentTime) { newIndex = i; break; }
    ...
    useEffect(() => {
        if (isAutoscrollActive && activeTranscriptIndex !== null && lineRefs.current[activeTranscriptIndex]) {
            ...
            if (scrollTimeoutRef.current) clearTimeout(scrollTimeoutRef.current);
            scrollTimeoutRef.current = window.setTimeout(() => { isProgrammaticScroll.current = false; }, 1000);
        }
    }, [activeTranscriptIndex, isAutoscrollActive]);

    const handleManualScroll = () => { if (isProgrammaticScroll.current) return; setIsAutoscrollActive(false); };

    const onPlayerStateChange = useCallback((event: YT.PlayerStateChangeEvent) => {
        modalRef.current?.focus();
        if (event.data === YT.PlayerState.PLAYING) startTranscriptSync(); else stopTranscriptSync();
        if (event.data === YT.PlayerState.CUED && autoplayModeRef.current !== 'off') event.target.playVideo();
        if (event.data === YT.PlayerState.ENDED) {
            if (autoplayModeRef.current === 'repeat') { event.target.seekTo(0, true); event.target.playVideo(); }
            else if (autoplayModeRef.current === 'on') { if (navigationRef.current.hasNextArticle) navigationRef.current.onNextArticle(); }
                ...
                const nextArticle = potentialNextArticles.length > 0 ? potentialNextArticles[Math.floor(Math.random() * potentialNextArticles.length)] : videoArticles[0];
    ...
    useEffect(() => { return () => { if (playerRef.current) { try { playerRef.current.destroy(); } catch (e) { console.error("Error destroying YouTube player:", e); } playerRef.current = null; } }; }, []);
            ...
            if (videoId) {
                ...
                if (playerRef.current) {
                    ...
                    playerRef.current = new YT.Player(playerElementId, {
                        videoId, playerVars: { controls: 1, rel: 0, showinfo: 0, iv_load_policy: 3, modestbranding: 1, origin: window.location.origin, autoplay: autoplayModeRef.current !== 'off' ? 1 : 0 },
                        events: {
    ...
    const parsedIframeContent = useMemo(() => {
        ...
        try {
            ...
            descriptionHtml = descriptionHtml.replace(/^(<br\s*\/?>\s*|<img[^>]*>\s*)+|(<br\s*\/?>\s*)+$/g, '').trim().replace(/^-/, '').trim();
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            ...
            if (copyDropdownRef.current && !copyDropdownRef.current.contains(event.target as Node)) setIsCopyDropdownOpen(false);
    ...
    const isImageInContent = useMemo(() => {
        ...
        try { let mainImageUrl: URL; try { mainImageUrl = new URL(article.imageUrl); } catch { return (article.content || article.description).includes(article.imageUrl); } const mainImageBaseUrl = mainImageUrl.origin + mainImageUrl.pathname; const tempDiv = document.createElement('div'); tempDiv.innerHTML = article.content || article.description; const imagesInContent = tempDiv.querySelectorAll('img'); for (const img of imagesInContent) { if (!img.src) continue; try { const contentImageUrl = new URL(img.src, article.link || window.location.href); const contentImageBaseUrl = contentImageUrl.origin + contentImageUrl.pathname; if (contentImageBaseUrl === mainImageBaseUrl) return true; } catch (e) { continue; } } } catch (e) { console.warn("Could not parse image URLs for comparison, falling back to string check.", e); return (article.content || article.description).includes(article.imageUrl); } return false;
    }, [article]);
    ...
    const handleToggleAutoscroll = (shouldBeActive: boolean) => {
        ...
        if (shouldBeActive && activeTranscriptIndex !== null && lineRefs.current[activeTranscriptIndex]) lineRefs.current[activeTranscriptIndex]?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    };

    const handleTimestampClick = (seconds: number) => { if (playerRef.current) { playerRef.current.seekTo(seconds, true); playerRef.current.playVideo(); } };
    ...
    const generateMarkdownForSummary = (): string => {
        ...
        if (summaryToDisplay.sections && summaryToDisplay.sections.length > 0) {
            ...
            markdown += summaryToDisplay.sections.map(section =>
                `- **${formatTranscriptTime(section.timestamp)} - ${section.title}**: ${section.summary}`
            ).join('\n') + '\n\n';
        }
        if (article?.sources && article.sources.length > 0) {
            markdown += `**Sources:**\n`;
            markdown += article.sources.map(source => `- [${source.title || source.uri}](${source.uri})`).join('\n') + '\n\n';
        }
        if (article?.link) {
    ...
    const generateHtmlForSummary = (): string => {
        ...
        if (summaryToDisplay.sections && summaryToDisplay.sections.length > 0) {
            ...
            html += summaryToDisplay.sections.map(section =>
        ...
        if (article?.sources && article.sources.length > 0) {
            ...
            html += article.sources.map(source => `<li><a href="${source.uri}" target="_blank" rel="noopener noreferrer">${source.title || source.uri}</a></li>`).join('');
            html += `</ul>`;
        }
        if (article?.link) {
    ...
    return (
        <div ref={modalRef} className={`${isTheaterMode ? 'bg-black' : 'bg-gray-800'} fixed inset-0 z-50 shadow-2xl flex flex-col overflow-hidden`} tabIndex={-1} onClick={e => e.stopPropagation()} onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onTouchEnd={handleTouchEnd} >
            {!isTheaterMode && (<header className={`p-4 border-b border-gray-700 flex-shrink-0 flex flex-col md:flex-row md:items-start gap-2 md:gap-4`}> <div className="flex-1 min-w-0 order-2 md:order-1"> <h2 className="text-lg md:text-xl font-bold text-white">{article.title}</h2>
                ...
                <div className="text-sm text-gray-400 mt-1 flex items-center flex-wrap gap-x-3 gap-y-1"> <button onClick={handleNavigateToFeed} className="hover:underline text-left" title={`Go to ${article.feedTitle} feed`}> {article.feedTitle} </button> <div className="flex items-center gap-1" title="Published Date"><CalendarIcon className="w-3 h-3" /><span>{formatRelativeDate(article.pubDateTimestamp)}</span></div> {article.isVideo && article.duration != null && (<div className="flex items-center gap-1 text-indigo-400 font-medium" title="Duration"><ClockIcon className="w-3 h-3" /><span>{formatTranscriptTime(article.duration)}</span></div>)} {article.views != null && <><span className="hidden sm:inline">&bull;</span><span className="flex items-center gap-1"><EyeIcon className="w-4 h-4" /> {formatViews(article.views)}</span></>} </div> </div> <div className="w-full md:w-auto flex-shrink-0 flex items-center justify-between order-1 md:order-2"> <div className="md:hidden w-6 h-6"></div> <div className="flex items-center gap-2"> <button onClick={articleNavigation.onPreviousArticle} onTouchStart={e => e.stopPropagation()} disabled={!articleNavigation.hasPreviousArticle} title="Previous Article (Left Arrow)" className="p-1 rounded-full text-gray-400 hover:text-white disabled:text-gray-600 disabled:cursor-not-allowed transition-colors"><ChevronLeftIcon className="w-6 h-6" /></button> <button onClick={articleNavigation.onNextArticle} onTouchStart={e => e.stopPropagation()} disabled={!articleNavigation.hasNextArticle} title="Next Article (Right Arrow)" className="p-1 rounded-full text-gray-400 hover:text-white disabled:text-gray-600 disabled:cursor-not-allowed transition-colors"><ChevronRightIcon className="w-6 h-6" /></button> {videoId && !isMobileLayout && <button onClick={() => setIsTheaterMode(true)} onTouchStart={e => e.stopPropagation()} title="Theater Mode" className="text-gray-400 hover:text-white p-1"><MaximizeIcon className="w-5 h-5" /></button>} <button onClick={handleClose} onTouchStart={e => e.stopPropagation()} title="Close (Esc)" className="text-gray-400 hover:text-white p-1"><XIcon className="w-6 h-6" /></button> </div> </div> </header>)}
            <div className={`flex-grow flex min-h-0 ${isMobileLayout ? 'flex-col' : 'flex-row'}`}>
                <div className="flex-grow flex flex-col min-h-0 min-w-0 relative">
                    <div className={`flex-grow overflow-y-auto ${isMobileLayout || isTheaterMode ? 'h-full' : ''}`}>
                        {isTheaterMode && (<div className="absolute top-2 right-2 z-20 flex items-center gap-2 bg-black/50 p-2 rounded-lg"> <button onClick={() => setIsTheaterMode(false)} onTouchStart={e => e.stopPropagation()} title="Exit Theater Mode" className="text-white hover:text-indigo-400 p-1"> <RestoreIcon className="w-6 h-6" /> </button> <button onClick={handleClose} onTouchStart={e => e.stopPropagation()} title="Close (Esc)" className="text-white hover:text-indigo-400 p-1"> <XIcon className="w-6 h-6" /> </button> </div>)}
                        {(videoId || article.hasIframe) ? (<div ref={videoPlayerContainerRef} className={`bg-black w-full relative ${isTheaterMode ? 'h-full aspect-auto' : isMobileLayout ? 'aspect-video' : 'h-full'}`}> {videoId && <div id="youtube-player-container" className="w-full h-full"></div>} {article.hasIframe && !videoId && (<div className="w-full h-full" dangerouslySetInnerHTML={{ __html: parsedIframeContent.playerHtml }} />)} </div>) : (<div className="article-content-area flex-grow overflow-y-auto px-6 pt-4 pb-12 h-full overflow-x-hidden" onTouchStart={() => { }}> {shouldShowImage && <img src={article.imageUrl!} alt={article.title} className="w-full max-w-2xl mx-auto h-auto object-cover rounded-lg mb-6 shadow-lg" />} {showLoadingDetails ? (<div className="flex flex-col items-center justify-center p-6 text-center text-gray-400 h-full"> <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500 mb-3"></div> <p className="font-semibold">Loading full video details...</p> </div>) : (<div className="prose prose-invert max-w-none prose-p:text-gray-300 prose-headings:text-gray-100 prose-a:text-indigo-400 prose-strong:text-gray-200" dangerouslySetInnerHTML={{ __html: processedContentForDisplay }} />)} </div>)}
                    </div>
                    <div className="flex-shrink-0 p-2 border-t border-gray-700 bg-gray-800/80 flex justify-center items-center gap-2 flex-wrap">
                        <Tooltip text={isRead ? 'Mark as Unread' : 'Mark as Read'} isMobile={isTouchDevice}><button onClick={() => article && handleToggleReadStatus(article.id)} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><span className="sr-only">{isRead ? 'Unread' : 'Read'}</span>{isRead ? <CheckCircleIcon className="w-5 h-5 text-indigo-400" /> : <EyeIcon className="w-5 h-5 text-gray-400 hover:text-white" />}</button></Tooltip>
                        <Tooltip text={isReadLater ? 'Remove from Read Later' : 'Save for Read Later'} isMobile={isTouchDevice}><button onClick={() => article && handleToggleReadLater(article.id)} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><span className="sr-only">{isReadLater ? 'Saved' : 'Save'}</span><BookmarkIcon className={`w-5 h-5 ${isReadLater ? 'fill-current text-indigo-400' : 'text-gray-400 hover:text-white'}`} /></button></Tooltip>
                        <Tooltip text="Tag this article" isMobile={isTouchDevice}><button onClick={() => setArticleToEditTags(article)} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><TagIcon className="w-5 h-5 text-gray-400 hover:text-white" /></button></Tooltip>
                        <Tooltip text="Remove Article" isMobile={isTouchDevice}><button onClick={() => article && handleRemoveArticle(article.id, article.feedId)} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><span className="sr-only">Remove Article</span><TrashIcon className="w-5 h-5 text-gray-400 hover:text-red-400" /></button></Tooltip>
                        {videoId && (<> <Tooltip text={isLiked ? 'Liked' : 'Like on YouTube'} isMobile={isTouchDevice}><button onClick={handleLikeClick} disabled={isLiked || !videoId} className="p-2.5 rounded-md transition-colors hover:bg-gray-700 flex-1 flex justify-center items-center disabled:cursor-not-allowed"><span className="sr-only">{isLiked ? 'Liked' : 'Like'}</span><LikeIcon className={`w-5 h-5 ${isLiked ? 'fill-current text-indigo-400' : 'text-gray-400 hover:text-white disabled:text-gray-600'}`} /></button></Tooltip> <div className="relative" ref={downloadDropdownRef}> <Tooltip text="Download" isMobile={isTouchDevice}><button onClick={() => setIsDownloadDropdownOpen(prev => !prev)} className="p-2.5 rounded-md transition-colors hover:bg-gray-700 flex-1 flex justify-center items-center disabled:cursor-not-allowed"><span className="sr-only">Download</span><DownloadIcon className="w-5 h-5 text-gray-400 hover:text-white" /></button></Tooltip> {isDownloadDropdownOpen && (<div className="absolute bottom-full mb-2 w-max bg-gray-700 rounded-md shadow-lg z-20 p-1 border border-gray-600"> {audioDownloadUrl && <a href={audioDownloadUrl} target="_blank" rel="noopener noreferrer" download className="w-full text-left flex items-center gap-2 p-2 rounded-md hover:bg-indigo-600 text-sm text-gray-200"><MusicIcon className="w-4 h-4" /><span>Audio (opus)</span></a>} {video1080pDownloadUrl && <a href={video1080pDownloadUrl} target="_blank" rel="noopener noreferrer" download className="w-full text-left flex items-center gap-2 p-2 rounded-md hover:bg-indigo-600 text-sm text-gray-200"><VideoIcon className="w-4 h-4" /><span>Video (1080p)</span></a>} {video360pDownloadUrl && <a href={video360pDownloadUrl} target="_blank" rel="noopener noreferrer" download className="w-full text-left flex items-center gap-2 p-2 rounded-md hover:bg-indigo-600 text-sm text-gray-200"><VideoIcon className="w-4 h-4" /><span>Audio+Video (360p)</span></a>} </div>)} </div> <Tooltip text="Autoplay next" isMobile={isTouchDevice}><button onClick={handleToggleAutoplayNext} className="p-2.5 rounded-md transition-colors hover:bg-gray-700 flex-1 flex justify-center items-center"><span className="sr-only">Autoplay Next</span><SkipForwardIcon className={`w-5 h-5 ${autoplayMode === 'on' ? 'text-indigo-400' : 'text-gray-400 hover:text-white'}`} /></button></Tooltip> <Tooltip text="Autoplay random" isMobile={isTouchDevice}><button onClick={handleToggleAutoplayRandom} className="p-2.5 rounded-md transition-colors hover:bg-gray-700 flex-1 flex justify-center items-center"><span className="sr-only">Autoplay Random</span><ShuffleIcon className={`w-5 h-5 ${autoplayMode === 'random' ? 'text-indigo-400' : 'text-gray-400 hover:text-white'}`} /></button></Tooltip> <Tooltip text="Repeat" isMobile={isTouchDevice}><button onClick={handleToggleAutoplayRepeat} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><span className="sr-only">Repeat</span><RepeatIcon className={`w-5 h-5 ${autoplayMode === 'repeat' ? 'text-indigo-400' : 'text-gray-400 hover:text-white'}`} /></button></Tooltip> </>)}
                        {article.link && <Tooltip text="Open Original" isMobile={isTouchDevice}><a href={article.link} target="_blank" rel="noopener noreferrer" className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center"><span className="sr-only">Open Original</span><ExternalLinkIcon className="w-5 h-5 text-gray-400 hover:text-white" /></a></Tooltip>}
                        <Tooltip text={summaryTooltipText} isMobile={isTouchDevice}><button onClick={handleSummarizeContent} disabled={isSummarizing || !canSummarize} className="p-2.5 rounded-md transition-colors hover:bg-gray-700/50 flex-1 flex justify-center items-center disabled:cursor-not-allowed"><span className="sr-only">{isSummarizing ? 'Generating...' : (summary || structuredSummary ? 'Regenerate' : 'Generate Summary')}</span><SparklesIcon className={`w-5 h-5 ${isSummarizing ? 'animate-pulse text-indigo-400' : 'text-gray-400 hover:text-white disabled:text-gray-600'}`} /></button></Tooltip>
                    </div>
                    {isPanelMinimized && !isMobileLayout && !isTheaterMode && (<div className="absolute top-0 right-0 h-full flex items-center z-10"> <Tooltip text="Show Panel" isMobile={isTouchDevice}> <button onClick={() => setIsPanelMinimized(false)} className="mr-1 p-1 rounded-full bg-gray-700/80 hover:bg-indigo-600 text-gray-300 hover:text-white transition-all shadow-lg" aria-label="Show Panel"> <ChevronLeftIcon className="w-5 h-5" /> </button> </Tooltip> </div>)}
                </div>
                {(isMobileLayout || (!isTheaterMode && !isPanelMinimized)) && (
                    <div className={`flex-shrink-0 bg-gray-800/50 flex flex-col ${isMobileLayout ? 'border-t-2 border-gray-700' : 'min-h-0 w-1/3 border-l-2 border-gray-700'}`} style={isMobileLayout ? { height: isMobilePanelMinimized ? 'auto' : `${panelHeight}px` } : {}} >
                        {isMobileLayout && (<div onMouseDown={!isMobilePanelMinimized ? handlePanelResizeStart : undefined} onTouchStart={!isMobilePanelMinimized ? handlePanelResizeStart : undefined} onDoubleClick={toggleMobilePanel} className={`flex-shrink-0 h-8 bg-gray-700 hover:bg-indigo-500 transition-colors ${!isMobilePanelMinimized ? 'cursor-row-resize' : 'cursor-pointer'} w-full flex items-center justify-center`}> {isMobilePanelMinimized ? <ChevronUpIcon className="w-6 h-6 text-gray-400" /> : <ChevronDownIcon className="w-6 h-6 text-gray-400" />} </div>)}
                        {(!isMobileLayout || !isMobilePanelMinimized) && (<> <div className="flex-shrink-0 p-3 border-b border-gray-700 flex items-center justify-between gap-2"> <div className="relative flex-grow" ref={dropdownRef}> <button onClick={() => setIsDropdownOpen(prev => !prev)} onTouchStart={e => e.stopPropagation()} className="w-full flex items-center justify-between px-3 py-1.5 bg-gray-700/50 rounded-md hover:bg-gray-700 transition-colors" > <div className="flex items-center gap-2"> {currentOption && <currentOption.icon className="w-5 h-5 text-indigo-400" />} <span className="font-semibold text-gray-200">{currentOption?.label}</span> </div> <ChevronDownIcon className={`w-5 h-5 text-gray-400 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} /> </button> {isDropdownOpen && (<div className="absolute top-full mt-1 w-full bg-gray-700 rounded-md shadow-lg z-20 p-1 border border-gray-600"> {dropdownOptions.map(option => (<button key={option.id} onClick={() => { setActiveTab(option.id); setIsDropdownOpen(false); }} className="w-full text-left flex items-center gap-2 p-2 rounded-md hover:bg-indigo-600 text-sm text-gray-200" > <option.icon className="w-5 h-5" /> <span>{option.label}</span> </button>))} </div>)} </div> {!isMobileLayout && (<Tooltip text="Hide Panel" isMobile={isTouchDevice}> <button onClick={() => setIsPanelMinimized(true)} className="p-1.5 rounded-full hover:bg-gray-700 text-gray-400 hover:text-white flex-shrink-0" aria-label="Hide Panel" > <ChevronRightIcon className="w-5 h-5" /> </button> </Tooltip>)} </div>
                            <div className="flex-grow overflow-y-auto p-4 article-content-area overflow-x-hidden" onScroll={handleManualScroll} ref={transcriptContainerRef}>
                                {activeTab === 'summary' && (() => {
                                    if (isSummarizing) {
                                        return (
                                        ...
                                        <div>
                                                ...
                                                <div className="relative" ref={copyDropdownRef}>
                                                    <Tooltip text="Copy Summary" isMobile={isTouchDevice}>
                                                        <button
                                                            ...
                                                            className="p-1.5 rounded-md transition-colors bg-gray-700 hover:bg-gray-600 disabled:opacity-50 disabled:cursor-not-allowed"
                                                        >
                                                            {copyStatus === 'idle' && <CopyIcon className="w-5 h-5 text-gray-300" />}
                                                            {copyStatus === 'copied' && <ClipboardCheckIcon className="w-5 h-5 text-green-400" />}
                                            ...
                                            {isTranslating ? (
                                                ...
                                                <>
                                                    ...
                                                    {summaryToDisplay.sections && summaryToDisplay.sections.length > 0 && (
                                                        ...
                                                        <div className="mt-4 pt-3 border-t border-gray-700/50">
                                                            ...
                                                            <ul className="space-y-3 text-sm">
                                                                {article.sources.map((source, index) => (
                                                                    <li key={index}>
                                                                        <a href={source.uri} target="_blank" rel="noopener noreferrer" className="font-semibold text-indigo-400 hover:underline hover:text-indigo-300 break-words" title={source.uri}>
                                                                            {index + 1}. {source.title || source.uri}
                                                                        </a>
                                                                        {source.description && (
                                                                            <p className="text-gray-400 mt-1 pl-4">{source.description}</p>
                                                                        )}
                                                                    </li>
                                                                ))}
                                                            </ul>
                                ...
                                {activeTab === 'transcript' && (isFetchingTranscript || isFetchingChoices ? (<div className="flex flex-col items-center justify-center h-full text-center text-gray-400"> <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500 mb-4"></div> <p className="font-semibold">{isFetchingChoices ? 'Finding transcripts...' : 'Loading transcript...'}</p> </div>) : transcriptError ? (<div className="bg-red-500/10 text-red-400 p-3 rounded-lg text-sm"> <p className="font-semibold mb-1">Error loading transcript:</p> <p>{transcriptError}</p> </div>) : transcriptData && transcriptData.transcript.length > 0 ? (<div> <div className="flex justify-between items-center mb-2 sticky top-0 bg-gray-800 py-2 z-10"> <div className="flex flex-wrap items-center gap-2"> {captionChoices && captionChoices.length > 1 && (<select value={selectedCaptionUrl || ''} onChange={(e) => setSelectedCaptionUrl(e.target.value)} className="bg-gray-700 text-white text-xs font-semibold py-1 pl-2 pr-6 rounded-md appearance-none focus:outline-none focus:ring-1 focus:ring-indigo-500" > {captionChoices.map(c => <option key={c.url} value={c.url}>{c.label}</option>)} </select>)} </div> <label htmlFor="autoscroll-toggle" className="flex items-center text-xs text-gray-400 cursor-pointer"> <input id="autoscroll-toggle" type="checkbox" checked={isAutoscrollActive} onChange={(e) => handleToggleAutoscroll(e.target.checked)} className="h-4 w-4 rounded border-gray-500 bg-gray-700 text-indigo-600 focus:ring-indigo-500" /> <span className="ml-2">Autoscroll</span> </label> </div> <div className="space-y-2"> {transcriptData.transcript.map((line, index) => (<div key={index} ref={el => { lineRefs.current[index] = el; }} onClick={() => handleTimestampClick(line.start)} className={`p-2 rounded-md cursor-pointer transition-colors text-sm ${activeTranscriptIndex === index ? 'bg-indigo-500/20' : 'hover:bg-gray-700/50'}`} dir={isRtl(transcriptData.language) ? 'rtl' : 'ltr'} > <span className="font-mono text-xs text-indigo-400 mr-2">{formatTranscriptTime(line.start)}</span> <span className={activeTranscriptIndex === index ? 'text-white' : 'text-gray-300'}>{line.text}</span> </div>))} </div> </div>) : null)}
                                {activeTab === 'comments' && (commentsState.isLoading ? (<div className="flex flex-col items-center justify-center h-full text-center text-gray-400"> <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500 mb-4"></div> <p className="font-semibold">Loading Comments...</p> </div>) : commentsState.error ? (<div className="bg-red-500/10 text-red-400 p-3 rounded-lg text-sm"> <p className="font-semibold mb-1">Error loading comments:</p> <p>{commentsState.error}</p> </div>) : commentsState.comments && commentsState.comments.length > 0 ? (<div className="divide-y divide-gray-700/50"> {commentsState.comments.map(comment => <CommentItem key={comment.commentId} comment={comment} />)} </div>) : (<div className="text-center text-gray-500 text-sm py-10">No comments found or comments are disabled.</div>))}
                                {activeTab === 'details' && (showLoadingDetails ? (<div className="flex flex-col items-center justify-center h-full text-center text-gray-400"> <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-indigo-500 mb-4"></div> <p className="font-semibold">Loading full video details...</p> </div>) : (<div className="prose prose-invert max-w-none prose-p:text-gray-300 prose-headings:text-gray-100 prose-a:text-indigo-400 prose-strong:text-gray-200" dangerouslySetInnerHTML={{ __html: article.hasIframe ? parsedIframeContent.descriptionHtml : processedContentForDisplay }} />))}
                            </div> </>)}
                    </div>
                )}
            </div>
