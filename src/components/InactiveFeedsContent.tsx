...
const FeedRow: React.FC<{
  ...
  return (
    <div
      className={`flex items-center gap-4 p-3 rounded-lg transition-colors ${isSelected ? 'bg-indigo-900/50' : 'bg-gray-800'}`}
    >
      ...
      <div className="flex-shrink-0">
        {feed.iconUrl && !iconError ? (
          ...
          <RssIcon className="w-10 h-10 text-gray-400" />
        )}
      </div>
      <div className="flex-grow min-w-0">
        <h3
          className={`font-bold text-lg text-gray-100 truncate ${hasError ? 'text-red-400' : ''}`}
          title={feed.title}
        >
          {feed.title}
        </h3>
        <p className="text-sm text-gray-400 truncate">
          {hasError ? (
            <span className="font-semibold" title={feed.error ?? ''}>
              {feed.error}
            </span>
          ) : hasNeverPosted ? (
            'Never posted'
          ) : (
            `Last post: ${formatRelativeDate(feed.lastPostTimestamp)}`
          )}
        </p>
      </div>
      <div className="flex-shrink-0">
...
export const InactiveFeedsContent: React.FC = () => {
  ...
  const isAllSelected = useMemo(
    () => feeds.length > 0 && selectedFeedIds.size === feeds.length,
        ...
        <div className="text-center mb-6 flex-shrink-0">
          ...
          <div className="flex justify-center items-center gap-2 flex-wrap">
            ...
            <FilterButton
              period="all"
              label="All"
              currentPeriod={inactivePeriod}
              setPeriod={setInactivePeriod}
            />
            <FilterButton
              period={1}
              label="1m"
