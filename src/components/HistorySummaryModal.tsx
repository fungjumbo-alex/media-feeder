...
import { XIcon, SparklesIcon, ChevronDownIcon, SaveIcon, CopyIcon, ClipboardCheckIcon } from './icons';
        ...
        <div className="py-6 border-b border-gray-700/50 last:border-b-0">
            ...
            {isExpanded && (
                <div className="mt-4">
                        ...
                        <div className="mt-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {group.keywords.map((keyword, kIndex) => (
                                    <span key={kIndex} className="px-2 py-1 text-xs font-medium bg-gray-700 text-gray-300 rounded-full">
                                        {keyword}
                                    </span>
                                ))}
                            </div>
                    ...
                    <h3 className="text-sm font-semibold text-gray-400 mt-4 mb-2">Related Articles ({group.articles.length}):</h3>
...
export const DigestModal: React.FC = () => {
    const { isDigestModalOpen, setIsDigestModalOpen, digestState, headerTitle, translateDigest, initialDigestLanguage, handleSaveDigestAsNote, articlesForDigest, handleQuotaError, handleSuccessfulApiCall } = useAppContext();
    ...
    const generateMarkdown = (): string => {
        ...
        if (digestType === 'detailed') {
            ...
            markdown += detailedDigest.map(item => {
                ...
                if (typeof item.summary === 'string') {
                    ...
                    if (item.summary.sections && item.summary.sections.length > 0) {
                        ...
                        itemText += item.summary.sections.map(section => 
                ...
                if (item.sources && item.sources.length > 0) {
                    itemText += `**Sources:**\n`;
                    itemText += item.sources.map(source => `- [${source.title || source.uri}](${source.uri})`).join('\n') + '\n\n';
                }
                return itemText;
            }).join('---\n\n');
        } else { // thematic
            ...
            markdown += thematicDigest.themedGroups.map(group => {
                ...
                groupText += group.articles.map(article => `- [${article.title}](${article.link})`).join('\n');
                return groupText;
            }).join('\n\n---\n\n');
        }
        return markdown;
        ...
        if (digestType === 'detailed') {
            ...
            html += detailedDigest.map(item => {
                ...
                if (item.sources && item.sources.length > 0) {
                    ...
                    itemHtml += item.sources.map(source => `<li><a href="${source.uri}">${source.title || source.uri}</a></li>`).join('');
                    itemHtml += `</ul>`;
                }
                return itemHtml;
            }).join('<hr>');
            ...
            html += thematicDigest.themedGroups.map(group => {
                ...
                groupHtml += group.articles.map(article => `<li><a href="${article.link}">${article.title}</a></li>`).join('');
                groupHtml += `</ul>`;
                return groupHtml;
            }).join('<hr>');
        }
    ...
    const renderContent = () => {
        ...
        if (isTranslating) {
            return ( <div className="flex flex-col justify-center items-center h-full text-center"> <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-indigo-500 mb-4"></div> <p className="text-lg text-gray-300">Translating to {selectedLanguage}...</p> <p className="text-sm text-gray-500">This may take a moment.</p> </div> );
        }
        if (translationError) return <ErrorDisplay message={translationError} />;
        if (!displayDigest || !digestType) return <div className="text-center text-gray-500">No digest available.</div>;

        if (digestType === 'thematic') {
            const thematicDigest = displayDigest as ThematicDigest;
            return (
                <div>
                    <h1 className="text-2xl font-bold text-white text-center mb-2">{thematicDigest.digestTitle}</h1>
                ...
                <div className="divide-y divide-gray-700/50">
                    {detailedDigest.map((item, index) => (
                        <div key={item.link || index} className="py-6">
                            ...
                            {item.link && ( <a href={item.link} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-400 hover:underline truncate block mb-4">{item.link}</a> )}
                            ...
                            {typeof item.summary === 'string' ? (
                                ...
                                <div>
                                    <div className="prose prose-invert max-w-none prose-p:my-2 prose-sm">
                                        {item.summary.overallSummary.split('\n').filter(p => p.trim() !== '').map((p, i) => <p key={i}>{p}</p>)}
                                    </div>
                                        ...
                                        <div className="mt-4 space-y-3">
                                            {item.summary.sections.map((section, secIndex) => (
                                                <div key={secIndex} className="bg-gray-900/50 p-3 rounded-md">
                                                    <p className="font-semibold text-indigo-400 text-sm"> <span className="font-mono">{formatTranscriptTime(section.timestamp)}</span> - {section.title} </p>
                                    ...
                                    <ol className="list-decimal list-inside space-y-2 text-sm">
                                        {item.sources.map((source, sourceIndex) => (
                                            <li key={sourceIndex} className="text-gray-400 truncate">
                                                <a href={source.uri} target="_blank" rel="noopener noreferrer" className="text-indigo-400 hover:underline hover:text-indigo-300" title={source.uri}> {source.title || source.uri} </a>
                                            </li>
                                        ))}
                                    </ol>
                                </div>
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-4xl h-[90vh] m-4 flex flex-col">
                ...
                <footer className="p-4 border-t border-gray-700 flex-shrink-0 bg-gray-800/50 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            onClick={() => displayDigest && handleSaveDigestAsNote(displayDigest, articlesForDigest)}
                        ...
                        <div className="relative" ref={copyDropdownRef}>
                            ...
                            >
                                ...
                                <ChevronDownIcon className={`w-4 h-4 ml-1 transition-transform ${isCopyDropdownOpen ? 'rotate-180' : ''}`} />
                            </button>
                            {isCopyDropdownOpen && (
