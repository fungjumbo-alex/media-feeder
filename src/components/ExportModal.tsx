...
export const ExportModal: React.FC = () => {
    const { 
        isExportModalOpen, setIsExportModalOpen, handleExportFeeds, allTags, 
        handleShareToCloudLink, feeds, setToast
    } = useAppContext();
            ...
            <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-lg m-4 relative flex flex-col max-h-[90vh] p-4 sm:p-6">
                ...
                <div className="flex-grow overflow-y-auto pr-2 -mr-2 space-y-4 sm:space-y-6">
                    <div>
                        ...
                        <p className="text-sm text-gray-400 mb-4">Generate a unique code to easily clone your setup to another device. The link expires in 24 hours.</p>
                        <ActionButton onClick={() => handleShareClick()} disabled={isSharing} className="bg-gray-700 hover:bg-gray-600 disabled:opacity-50">
                        ...
                        {shareCode && shareLink && (
                            <div className="mt-4 bg-gray-900/50 p-4 rounded-lg flex flex-col items-center gap-4">
                                ...
                                <div className="w-full flex items-center gap-2">
                                    <input type="text" value={shareCode} readOnly className="w-full bg-gray-700 border border-gray-600 text-gray-200 text-sm rounded-md px-2 py-1.5 focus:outline-none text-center font-mono tracking-wider" />
                                    <button onClick={handleCopyShareCode} className="flex-shrink-0 flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-semibold bg-gray-600 hover:bg-gray-500 text-white">
                                        {copyCodeButtonText === 'Copied!' ? <ClipboardCheckIcon className="w-4 h-4" /> : <CopyIcon className="w-4 h-4" />}
                                        {copyCodeButtonText}
                                    </button>
                                </div>
                                ...
                                <button onClick={handleCopyLink} className="text-xs text-gray-500 hover:text-gray-300 underline">
                                    {copyLinkButtonText}
                                </button>
                            </div>
                        )}
                    ...
                    <div>
                        ...
                        <ActionButton onClick={() => handleExportFeeds({})} className="bg-gray-700 hover:bg-gray-600">
                            Download Full Backup
                        </ActionButton>
                    </div>

                    <div>
                        <h3 className="text-base font-semibold text-gray-200 mb-2">Subscriptions Only (OPML)</h3>
                        <p className="text-sm text-gray-400 mb-4">Saves just your feed subscriptions to a standard <code>.opml</code> file, compatible with other feed readers.</p>
                    ...
                    {allTags.length > 0 && (
                        <div>
                            ...
                            <p className="text-sm text-gray-400 mb-4">Export or share only the feeds that have a specific tag.</p>
                            ...
                            {selectedTagForExport && (
                                <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <ActionButton onClick={() => handleExportFeeds({ tag: selectedTagForExport })} className="bg-gray-700 hover:bg-gray-600">
                                        <ExportIcon className="w-5 h-5" />
                                        Download File
                                    </ActionButton>
                                    <ActionButton onClick={() => handleShareClick(selectedTagForExport)} disabled={isSharing} className="bg-indigo-600 hover:bg-indigo-500 disabled:bg-indigo-400/50">
                                        {isSharing && sharingTag === selectedTagForExport ? (
                                            <svg className="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                                        ) : <LinkIcon className="w-5 h-5" />}
                                        {isSharing && sharingTag === selectedTagForExport ? 'Generating...' : 'Share Link'}
                                    </ActionButton>
                                </div>
                            )}
                        </div>
                ...
                <footer className="mt-4 pt-4 border-t border-gray-700 flex-shrink-0 flex justify-end">
                    <button onClick={onClose} className="px-4 py-2 rounded-md text-gray-300 bg-gray-700 hover:bg-gray-600">Close</button>
                </footer>
            </div>
        </div>
    );
