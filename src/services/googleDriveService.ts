...
const handleDriveError = async (response: Response, defaultMessage: string): Promise<never> => {
  ...
  if (response.status === 401 || response.status === 403) {
    ...
    if (response.status === 403) {
      const hasInsufficientPerms = errorBody?.error?.errors?.some(
        (e: any) => e.reason.includes('insufficientPermissions') || e.reason.includes('forbidden')
      );
...
export const loadDataFromDrive = async (
  accessToken: string
): Promise<{ data: SyncData | null; metadata: { id: string; modifiedTime: string } | null }> => {
