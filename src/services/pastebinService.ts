...
export const fetchContentFromPastebinUrl = async (pastebinUrl: string): Promise<string> => {
    ...
    try {
        ...
        throw new Error(`Could not fetch content from Pastebin. The link might be invalid or the service is down. Error: ${error instanceof Error ? error.message : String(error)}`);
    }
};
