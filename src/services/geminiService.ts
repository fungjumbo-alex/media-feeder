...
export const fetchAndParseTranscript = async (url: string): Promise<TranscriptLine[]> => {
  ...
  return await fetchTranscript(
    url.startsWith('backend-transcript:')
...
export const generateMindmapHierarchy = async (
  ...
  console.log(`[AI Clustering] Processing ${articles.length} ${groupName} articles`);
...
3. Group remaining articles into general topics (3-8 topics total)`;
      ...
      console.error(`Failed to generate ${groupName} hierarchy:`, error);
      throw error;
    }
  }
