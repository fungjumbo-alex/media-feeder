...
const getApiKey = (): string => {
    ...
    if (!key) throw new Error("Live Sync feature is not configured by the app developer (missing PASTEGG_API_KEY).");
    return key;
};
...
export const createPaste = async (content: string, title: string): Promise<string> => {
    ...
    const responseText = await fetchViaProxy(PASTEGG_API_BASE, 'rss', undefined, undefined, undefined, undefined, fetchOptions);
...
export const deletePaste = async (pasteId: string): Promise<void> => {
    ...
    await fetchViaProxy(`${PASTEGG_API_BASE}/${pasteId}`, 'rss', undefined, undefined, undefined, undefined, fetchOptions);
};
 ...
 * @returns The ID of the newly created paste.
 */
export const updatePaste = async (oldPasteId: string, newContent: string, newTitle: string): Promise<string> => {
    ...
    try {
        ...
        console.warn(`Could not delete old paste ${oldPasteId} during update, a new one will be created anyway.`, e);
    }
