...
const handleYouTubeError = async (response: Response, defaultMessage: string): Promise<never> => {
    ...
    if (response.status === 401) {
        const authError = new Error("Authentication failed. Your session may have expired. Please try signing in again.");
...
export const fetchYouTubeComments = async (videoId: string): Promise<YouTubeComment[]> => {
    ...
    if (lastError) {
        ...
        throw new Error(`All Invidious instances failed to fetch comments. Last error: ${errorMessage}`);
    }
...
export const fetchYouTubeVideoDetails = async (videoId: string): Promise<{ description: string; views: number | null }> => {
    ...
    const errorMessage = lastError instanceof Error ? lastError.message : String(lastError);
    throw new Error(`All Invidious instances failed to fetch video details. Last error: ${errorMessage}`);
};
...
export const fetchYouTubeSubscriptions = async (accessToken: string): Promise<YouTubeSubscription[]> => {
    ...
    do {
        ...
        const subs = (data.items || []).map((item): YouTubeSubscription => ({
            id: item.id,
    ...
    do {
        ...
        const items = (itemsData.items || []).map((item): Article => {
            ...
            return {
                ...
                imageUrl: snippet.thumbnails.maxres?.url || snippet.thumbnails.high?.url || snippet.thumbnails.medium?.url || snippet.thumbnails.default?.url || null,
        ...
        try {
            ...
            if (data && data.videoId) {
                return {
                    ...
                    imageUrl: data.videoThumbnails?.find((t) => t.quality === 'maxres')?.url || data.videoThumbnails?.[0]?.url,
